...
def convert():    
    ...
    for pathnum in range(listbox.size()):
        ...
        if path.endswith((".pdf",".PDF")) :      #or str.endswith(path, ".PDF"):
        ...
        elif path.endswith((".imgdoc",".docx",'DOC','DOCX','wps','WPS')):      #str.endswith(path,".imgdoc") or str.endswith(path, ".docx") or str.endswith(path, ".wps") or str.endswith(path,".DOC") or str.endswith(path, ".DOCX") or str.endswith(path, ".WPS"):
            ...
            office = comtypes.client.CreateObject("Powerpoint.Application")
            ...
            current_statusinfo='不支持文件格式：',inputfile_name        
            update_currentstatus(current_statusinfo)

    output_filename = en1.get()     #手动输入文件名
    if len(pdf_path_list):
        ...
        pdf_writer=merge_pdfs(pdf_path_list)
...
label1_1=Label(labelframe1, text='请把需要合并的word/excel/ppt/pdf文档拖拽到下方。').pack(side=LEFT,padx=5)
...
Button(buttonbox1, text='测试', width=10,command=test).pack(side=TOP, padx=5,pady=5)       #测试用
...
checkb2=Checkbutton(checkframe3, text = "添加空白页在总页数为基数的pdf文件末尾", variable = CheckVar2, \
